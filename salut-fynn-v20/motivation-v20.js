...
let audioCtx=null;
const TARGET_RMS=.24;   // kräftige Sprachlautstärke
const MAX_GAIN=32;      // bis ca. +30 dB für sehr leise Aufnahmen
function loadScript(src){return new Promise((res,rej)=>{const s=document.createElement('script');s.src=base+src+'?v=20e';s.async=false;s.onload=res;s.onerror=()=>rej(new Error(src));document.head.appendChild(s)})}
...
function measure(buf){let peak=0,sum=0,n=0;const step=Math.max(1,Math.floor(buf.length/180000));for(let ch=0;ch<buf.numberOfChannels;ch++){const d=buf.getChannelData(ch);for(let i=0;i<d.length;i+=step){const a=Math.abs(d[i]);if(a>peak)peak=a;if(a>.006){sum+=a*a;n++}}}const rms=n?Math.sqrt(sum/n):peak;let gain=rms>0?TARGET_RMS/rms:1;gain=Math.max(1,Math.min(MAX_GAIN,gain));return{peak,rms,gain}}
async function decodeAll(){if(decoded)return true;if(decoding)return decoding;decoding=(async()=>{const c=ctx();const out=[];for(const item of raw){const ab=b64ToBuffer(item);out.push(await c.decodeAudioData(ab.slice(0)))}buffers=out;decoded=buffers.length===7;return decoded})().catch(e=>{console.warn('Papa-Audio konnte nicht dekodiert werden',e);decoded=false;return false}).finally(()=>{decoding=null});return decoding}
...
async function playPapa(force=false){if(!ready||(playing&&!force))return false;await unlock();if(!decoded){const ok=await decodeAll();if(!ok){pending=true;return false}}const i=pick();try{if(window.speechSynthesis)window.speechSynthesis.cancel();if(currentSource){try{currentSource.stop()}catch(e){}}const c=ctx(),src=c.createBufferSource(),gain=c.createGain(),comp=c.createDynamicsCompressor();const m=measure(buffers[i]);gain.gain.value=m.gain;comp.threshold.value=-10;comp.knee.value=12;comp.ratio.value=12;comp.attack.value=.002;comp.release.value=.18;src.buffer=buffers[i];src.connect(gain).connect(comp).connect(c.destination);src.onended=finishPapa;currentSource=src;playing=true;pending=false;last=i;reset();badge();src.start(0);console.log('Papa-Audio',i+1,'RMS',m.rms.toFixed(4),'Gain',m.gain.toFixed(1));return true}catch(e){playing=false;pending=true;console.warn('Papa-Audio Fehler',e);return false}}
...
function install(){
 ...
 const manage=document.querySelector('#manage .panel');if(manage&&!manage.querySelector('.papaAudioTestBtn')){const b=document.createElement('button');b.className='secondary papaAudioTestBtn';b.style.marginTop='8px';b.textContent='💙 Papa-Aufnahme testen (laut)';b.onclick=async()=>{if(!ready){if(typeof toastMsg==='function')toastMsg('Papa-Aufnahmen laden noch …');return}if(typeof toastMsg==='function')toastMsg('Papa-Aufnahme wird laut abgespielt …');await playPapa(true)};manage.appendChild(b)}
 window.playPapaMotivation=()=>playPapa(true);window.__PAPA_STATUS=()=>({ready,clips:raw.length,decoded,buffers:buffers.length,playing,pending,tasks,nextAt,last,audioContext:audioCtx?.state||'none'});
}
(async()=>{try{window.PAPA_CLIPS=[];window.PAPA_B64={};for(const f of files)await loadScript(f);raw=[...window.PAPA_CLIPS];for(const i of [4,5,6,7])if(window.PAPA_B64[i])raw.push('data:audio/mp4;base64,'+window.PAPA_B64[i]);if(raw.length!==7)throw new Error('Erwartet 7 Aufnahmen, geladen '+raw.length);ready=true;install();console.log('Papa-Motivation bereit:',raw.length)}catch(e){console.warn('Papa-Motivation konnte nicht initialisiert werden',e)}})();
})();
