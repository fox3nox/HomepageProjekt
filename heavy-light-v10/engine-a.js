...
function resetRun(){
 ...
 shake=0;flash=0;particles=[];sparks=[];rings=[];trails=[];shockwaves=[];held=false;finished=false;paused=false;failLock=0;hitStop=0;landingPulse=0;
...
function jumpNow(power=-980){player.vy=power;player.grounded=false;coyote=0;jumpBuffer=0;airRecoverUsed=false;shock(player.x,player.y+player.r,'#8cf8ff');emit(player.x,player.y+player.r,'#8cf8ff',20,245,110);shake=Math.max(shake,4.2);tone(720,.045,'triangle',.026);tone(360,.05,'sine',.012)}
function setMode(h,fx=true){
 ...
 if(!h&&wasHeavy){
   jumpBuffer=.15;
   if(player.grounded||coyote>0)jumpNow(-1025);
   else if(!airRecoverUsed){player.vy=Math.max(-225,player.vy-230);airRecoverUsed=true;jumpBuffer=0}
 }else if(h&&!wasHeavy&&!player.grounded){player.vy+=350;airRecoverUsed=true}
 if(fx){
   ...
   if(d<.105){chargeFlow(8);flash=Math.max(flash,.10);toast(h?'SYNC DROP':'SYNC LIFT');tone(h?220:880,.045,'triangle',.018)}
 }
}
