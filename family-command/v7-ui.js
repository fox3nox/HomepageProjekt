...
(()=>{
  ...
  function compact(root){
    ...
    root.querySelectorAll('.v6-kid,.fc-tom-screen-kid').forEach(card=>{card.classList.add('v7-kid-row');const chip=card.querySelector('.v6-chip,.fc-tom-screen-kid-head>span:last-child');if(chip&&/Schule/i.test(chip.textContent||''))chip.remove();const grid=card.querySelector('.v6-time-grid');if(grid&&!card.querySelector('.v7-kid-line')){const cells=[...grid.querySelectorAll('.v6-time-cell')],v=i=>cells[i]?.querySelector('b')?.textContent?.trim()||'—',line=document.createElement('div');line.className='v7-kid-line';line.innerHTML=`<strong>${v(0)} los</strong><span>${v(1)}–${v(2)}</span>`;grid.replaceWith(line)}const free=card.querySelector('.v6-kid-free');if(free){free.classList.add('v7-free');if(/keine feste/i.test(free.textContent||''))free.textContent='Frei · keine feste Schulzeit'}const freeP=card.matches('.fc-tom-screen-kid')?card.querySelector(':scope>p'):null;if(freeP&&/keine feste/i.test(freeP.textContent||'')){freeP.classList.add('v7-free');freeP.textContent='Frei · keine feste Schulzeit'}});
