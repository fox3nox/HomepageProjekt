...
(()=>{'use strict';
 ...
 function eventState(e){const now=new Date(),last=e.endDate||e.date,start=e.time?new Date(e.date+'T'+e.time+':00'):null,end=e.end?new Date(last+'T'+e.end+':00'):null;if(last<today()||(end&&end<=now))return 'past';if(start&&start<=now){if(end&&end>now)return 'running';if(last===today())return 'started';if(last>today())return 'spanning'}return e.time?'upcoming':'untimed'}
 function exact(e){const short=d=>new Date(d+'T12:00:00').toLocaleDateString('de-CH',{day:'2-digit',month:'2-digit'});return e.endDate&&e.endDate!==e.date?[short(e.date),e.time,'–',short(e.endDate),e.end].filter(Boolean).join(' '):e.time?[e.time,e.end].filter(Boolean).join('–'):'Ohne Uhrzeit'}
 ...
 window.__fcClarity={ids,matches,who,filters,eventState,exact,next};
})();
