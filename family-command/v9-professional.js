...
function bindSummary(summary){
  ...
  for(const el of summary.querySelectorAll('.fc31-stat')){const action=el.dataset.fc31==='events'?openCalendar:openTasks;el.setAttribute('role','button');el.tabIndex=0;el.onclick=action;el.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();action()}}}
}
...
function decorateTasks(sec){if(!sec)return;const rows=[...sec.querySelectorAll('[data-todo]')];for(const row of rows){row.classList.add('fc33-task-row');if(!row.querySelector('.fc33-task-icon')){const title=row.querySelector('.fc9-row-main b')?.textContent||'';const icon=document.createElement('span');icon.className='fc33-task-icon';icon.setAttribute('aria-hidden','true');icon.textContent=taskIcon(title);row.querySelector('.fc9-check')?.after(icon)}}let kicker=sec.querySelector('.fc33-kicker');if(!kicker){kicker=document.createElement('div');kicker.className='fc33-kicker';sec.querySelector('.fc9-section-head')?.after(kicker)}kicker.innerHTML=`<span>NOCH ERLEDIGEN</span><span class="fc33-count">${rows.length}</span>`}
...
function reorder(page,focus,summary,todoSec,hwSec,eventSec,peopleSec,tomSec){const ordered=[focus&&!focus.classList.contains('fc31-focus-done')?focus:null,todoSec,hwSec,eventSec,peopleSec,tomSec].filter(Boolean);let anchor=summary;for(const node of ordered){anchor.after(node);anchor=node}}
function enhanceToday(){
  if(innerWidth>719)return;
  const root=document.getElementById('today');if(!root||!root.classList.contains('active'))return;
  const page=root.querySelector('.fc9-page');if(!page)return;ensureStyle();
  ...
  const head=page.querySelector('.fc9-pagehead');if(head&&!head.querySelector('.fc33-tagline')){const t=document.createElement('span');t.className='fc33-tagline';t.textContent='Das Wichtigste zuerst';head.querySelector('h1')?.after(t)}
  let summary=page.querySelector('.fc31-summary');if(!summary){summary=document.createElement('div');summary.className='fc31-summary';summary.innerHTML='<div class="fc31-stat" data-fc31="important"><strong>0</strong><span>Wichtig</span></div><div class="fc31-stat" data-fc31="open"><strong>0</strong><span>Offen</span></div><div class="fc31-stat" data-fc31="events"><strong>0</strong><span>Termine</span></div>';head?.after(summary);bindSummary(summary)}
  const importantStat=summary.querySelector('[data-fc31="important"]');importantStat?.classList.toggle('urgent',important>0);setText(importantStat?.querySelector('strong'),important);setText(summary.querySelector('[data-fc31="open"] strong'),todos.length);setText(summary.querySelector('[data-fc31="events"] strong'),events);
  ...
  const todoSec=sectionBy(page,'[data-todo]'),hwSec=sectionBy(page,'[data-hw]'),eventSec=sectionBy(page,'[data-event]'),peopleSec=sectionBy(page,'.fc9-person-list'),tomSec=sectionBy(page,'.fc9-tomorrow');
  if(todoSec){todoSec.classList.add('fc33-priority-section');const h=todoSec.querySelector('.fc9-section-head h2');setText(h,'Heute – Das Wichtigste zuerst');decorateTasks(todoSec)}
  if(hwSec){hwSec.classList.add('fc33-homework-section');setText(hwSec.querySelector('.fc9-section-head h2'),'Schulaufgaben')}
  if(eventSec){eventSec.classList.add('fc33-overview-section');setText(eventSec.querySelector('.fc9-section-head h2'),'Heute im Überblick')}
  if(peopleSec){peopleSec.classList.add('fc33-family-section');setText(peopleSec.querySelector('.fc9-section-head h2'),'Schule & Familie heute');decoratePeople(peopleSec)}
  if(tomSec){tomSec.classList.add('fc33-tomorrow-section');setText(tomSec.querySelector('.fc9-section-head h2'),'Morgen – Vorschau')}
  reorder(page,focus,summary,todoSec,hwSec,eventSec,peopleSec,tomSec);
  document.documentElement.dataset.fcIphoneDashboard='v31';document.documentElement.dataset.fcIphoneDensity='v32';document.documentElement.dataset.fcReferenceDesign='v33';
}
