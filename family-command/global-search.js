...
function index({history=false}={}){
  ...
  for(const x of window.__fcDocumentLibrary?.cached?.()||[])add('Dokumente',x.id,x.title,names(x.personIds),[x.summary,x.description,x.document_type,x.mime_type,rows(x.tags).map(t=>typeof t==='string'?t:t.name),x.metadata?JSON.stringify(x.metadata):''],()=>window.fcOpenOriginal?.(x.id),x.personIds);
