...
const groups=['Termine','Aufgaben','Hausaufgaben','Personen','Dokumente','Erinnerungen','Notizen','Kontakte','Einkauf','Rezepte','Pendenzen'];
function index({history=false}={}){
  const out=[],d=D(),add=(group,id,title,meta,text,action,personIds=[])=>{if(title)out.push({group,id:String(id),title:String(title),meta:String(meta||''),text:norm([title,meta,text].flat(Infinity).join(' ')),action,personIds})};
  for(const x of rows(d.events).filter(active))if(history||String(x.endDate||x.date||'')>=today())add('Termine',x.id,x.title,[names(x.personIds),x.date<today()&&x.endDate>=today()?'Läuft':history&&x.date<today()?x.date:dateLabel(x.date),x.time].filter(Boolean).join(' · '),x.note,()=>window.fcOpenEventDetails?.(x.id),x.personIds);
  ...
  for(const [id,x] of todos)if(active(x)&&(history||!x.done))add('Aufgaben',id,x.title,[name(x.personId),x.done?'Erledigt':dateLabel(x.date)].filter(Boolean).join(' · '),x.note,()=>window.__fcV9?.editTodo?.(id),[x.personId]);
  for(const x of rows(d.homework).filter(active))if(history||!x.done)add('Hausaufgaben',x.id,[x.subject,x.title].filter(Boolean).join(' · '),[name(x.personId),x.done?'Erledigt':dateLabel(x.dueDate)].filter(Boolean).join(' · '),x.note,()=>window.__fcV9?.editHomework?.(x.id),[x.personId]);
  ...
  for(const x of window.__fcDocumentLibrary?.cached?.()||[])add('Dokumente',x.id,x.title,names(x.personIds),[x.summary,x.description,x.searchText,x.document_type,x.mime_type,rows(x.tags).map(t=>typeof t==='string'?t:t.name),x.metadata?JSON.stringify(x.metadata):''],()=>window.fcOpenOriginal?.(x.id),x.personIds);
  ...
  for(const x of rows(d.reminders).filter(active)){const dayNames=['So','Mo','Di','Mi','Do','Fr','Sa'],days=(x.days||[]).map(d=>dayNames[Number(d)]).filter(Boolean).join(' · ');add('Erinnerungen',x.id,(x.items||[]).join(' · '),[name(x.personId),days].filter(Boolean).join(' · '),'',()=>window.fcOpenReminderCenter?.(),[x.personId]);}
