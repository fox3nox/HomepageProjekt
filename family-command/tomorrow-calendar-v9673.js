...
function openCalendarDate(date){const state=window.__fcV9?.state;if(!state||!/^\d{4}-\d{2}-\d{2}$/.test(String(date||'')))return false;state.weekDate=String(date);state.calendarMode='week';window.__fcV9.invalidate?.('events');window.__fcV9.render?.('events',true);return true}
...
function install(){const app=document.getElementById('fcApp')||document.body;obs.observe(app,{childList:true,subtree:true});document.addEventListener('click',e=>{const day=e.target.closest?.('[data-fc673-date]');if(day){e.preventDefault();e.stopPropagation();openCalendarDate(day.dataset.fc673Date);return}if(e.target.closest?.('[data-screen="tomorrow"],[data-screen="events"]'))setTimeout(run,30)},true);run()}
