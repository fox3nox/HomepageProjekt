...
function enhanceCalendar(){
  ...
  if(grid)grid.outerHTML=html;else{month.insertAdjacentHTML('afterend',`<details class="fc-calendar-disclosure" ${window.__fcV9.state.calendarExpanded?'open':''}><summary aria-label="Monat auswählen">${fcIcon('calendar')}<span>Monat auswählen</span></summary>${html}</details>`);const details=root.querySelector('.fc-calendar-disclosure');details.addEventListener('toggle',()=>{window.__fcV9.state.calendarExpanded=details.open})}
