...
try {
  ...
  assert.ok((await page.evaluate(()=>caches.keys())).includes('family-command-v147-explicit-work-prep'));
  const offlineAssets=await page.evaluate(async()=>{const cache=await caches.open('family-command-v147-explicit-work-prep');return Promise.all(['v11-app.js','v11.css'].map(async name=>({name,cached:!!await cache.match('/'+name)})))});
