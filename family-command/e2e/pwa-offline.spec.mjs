...
try {
  ...
  assert.ok((await page.evaluate(()=>caches.keys())).includes('family-command-v142-workplan-health'));
  const offlineAssets=await page.evaluate(async()=>{const cache=await caches.open('family-command-v142-workplan-health');return Promise.all(['v11-app.js','v11.css'].map(async name=>({name,cached:!!await cache.match('/'+name)})))});
