...
try{
 ...
 console.log('global-design-v40',JSON.stringify(metrics));assert.equal(before,after,'visual design must not mutate family state');
 for(const [id,m] of Object.entries(metrics)){assert.equal(m.active,true,`${id} must render`);assert.equal(m.overflow,true,`${id} must not overflow horizontally`);if(id!=='today')assert.ok(m.h1>0&&m.h1<=26,`${id} page title too large: ${m.h1}`);assert.ok(m.maxCardRadius<=12,`${id} cards should stay compact: ${m.maxCardRadius}`);assert.ok(m.mainPadding<=11,`${id} mobile gutters too wide: ${m.mainPadding}`)}
 assert.ok(metrics.tomorrow.maxRow===0||metrics.tomorrow.maxRow<=68,`tomorrow rows too tall: ${metrics.tomorrow.maxRow}`);assert.ok(metrics.events.maxRow===0||metrics.events.maxRow<=60,`calendar rows too tall: ${metrics.events.maxRow}`);assert.ok(metrics.homework.maxRow===0||metrics.homework.maxRow<=68,`task rows too tall: ${metrics.homework.maxRow}`);
 await browser.close();console.log('V9.40 global mobile design regression: ok');
