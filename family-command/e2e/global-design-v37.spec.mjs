...
try{
 ...
 for(const [id,m] of Object.entries(metrics)){assert.equal(m.active,true,`${id} must render`);assert.equal(m.overflow,true,`${id} must not overflow horizontally`);if(id!=='today')assert.ok(m.h1>0&&m.h1<=28,`${id} page title too large: ${m.h1}`);assert.ok(m.maxCardRadius<=14,`${id} cards should stay compact: ${m.maxCardRadius}`);assert.ok(m.mainPadding<=12,`${id} mobile gutters too wide: ${m.mainPadding}`)}
 assert.ok(metrics.tomorrow.maxRow===0||metrics.tomorrow.maxRow<=84,`tomorrow rows too tall: ${metrics.tomorrow.maxRow}`);assert.ok(metrics.events.maxRow===0||metrics.events.maxRow<=84,`calendar rows too tall: ${metrics.events.maxRow}`);assert.ok(metrics.homework.maxRow===0||metrics.homework.maxRow<=84,`task rows too tall: ${metrics.homework.maxRow}`);
