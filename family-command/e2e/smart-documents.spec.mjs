...
try{
 ...
 try{
   ...
   const diagnostic=await page.evaluate(()=>({events:window.data?.events,homework:window.data?.homework,todos:window.data?.todos,status:document.querySelector('[data-doc-status]')?.textContent,checked:[...document.querySelectorAll('[data-doc-person]:checked')].map(x=>x.value),health:window.__fcSmartDocumentsHealth,access:localStorage.getItem('fc-private-access-v1')}));
   console.log('SMART_DOCUMENTS_DIAGNOSTIC',JSON.stringify(diagnostic));
   throw e;
 }
