...
try{
  ...
  await page.waitForFunction(()=>[...document.querySelectorAll('#today .fc38-event-main .fc52-all-day')].some(x=>x.textContent==='Ganztägig'));
  const m=await page.evaluate(()=>{
    ...
    const allDay=[...document.querySelectorAll('#today .fc38-event-main .fc52-all-day')];
  ...
  assert.equal(m.polish,'v52');assert.equal(m.dashboard,'v52');assert.equal(m.legacyPolish,'v49');assert.equal(m.legacyDashboard,'v40');
