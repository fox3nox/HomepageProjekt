...
try{
  ...
  await page.waitForFunction(()=>document.documentElement.dataset.fcMobilePolish==='v52'&&document.documentElement.dataset.fcMobileDashboardPolish==='v53',{timeout:20000});
  await page.evaluate(()=>{const iso=typeof todayISO==='function'?todayISO():new Date().toISOString().slice(0,10);window.data.events=Array.isArray(window.data.events)?window.data.events:[];window.data.events.push({id:'v952-all-day',date:iso,time:'',title:'V9.52 Ganztagstest',personIds:['jayden'],note:''});window.__fcReferenceDashboard40?.rebuild(true)});
  ...
  assert.equal(m.polish,'v52');assert.equal(m.dashboard,'v53');assert.equal(m.legacyPolish,'v49');assert.equal(m.legacyDashboard,'v40');
