...
const toggle=scripts.find(s=>String(s.src).includes('today-overview-toggle-v9668.js'));assert.ok(toggle,'today overview toggle loaded');assert.equal(new URL(toggle.src,base).searchParams.get('v'),'20260906-v9669');
const future=scripts.find(s=>String(s.src).includes('future-only-polish-v9669.js'));assert.ok(future,'future-only polish loaded');assert.equal(new URL(future.src,base).searchParams.get('v'),'20260906-v9669');
