...
async function snapCore(page,prefix){for(const id of ['today','tomorrow','events','homework','more']){await page.locator(`.fc9-nav button[data-screen="${id}"]`).click();await page.waitForTimeout(120);const m=await page.evaluate(id=>{const root=document.getElementById(id),nav=document.querySelector('.fc9-nav');return{id,active:root.classList.contains('active'),overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth+1,navBottom:nav.getBoundingClientRect().bottom,viewportHeight:window.innerHeight,h1:parseFloat(getComputedStyle(root.querySelector('.fc9-pagehead h1')||root.querySelector('h1')).fontSize)||0}},id);assert.equal(m.active,true);assert.equal(m.overflow,false,`${id} must not overflow`);assert.ok(m.navBottom<=m.viewportHeight+1,`${id} navigation must stay inside viewport`);await page.screenshot({path:`${OUT}/${prefix}-${id}.png`,fullPage:false})}}
