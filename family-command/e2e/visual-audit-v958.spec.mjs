...
async function boot(browser,viewport,mobile=false){const ctx=await browser.newContext({viewport,isMobile:mobile,hasTouch:mobile,serviceWorkers:'block'});await ctx.addInitScript({content:seed});const page=await ctx.newPage();await page.goto(BASE+'/?access=test',{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>document.documentElement.dataset.fcReady==='1');await page.waitForFunction(()=>document.documentElement.dataset.fcUnifiedDesign==='v58');await page.waitForFunction(()=>[...document.styleSheets].some(s=>String(s.href||'').includes('v9-unified-design-v958.css')));return{ctx,page}}
async function snapCore(page,prefix,layout){const renderedHeight=page.viewportSize()?.height||0;for(const id of ['today','tomorrow','events','homework','more']){await page.locator(`.fc9-nav button[data-screen="${id}"]`).click();await page.waitForTimeout(120);const m=await page.evaluate(id=>{const root=document.getElementById(id),nav=document.querySelector('.fc9-nav'),r=nav.getBoundingClientRect();return{id,active:root.classList.contains('active'),overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth+1,navTop:r.top,navBottom:r.bottom,navHeight:r.height,navWidth:r.width,navPosition:getComputedStyle(nav).position,h1:parseFloat(getComputedStyle(root.querySelector('.fc9-pagehead h1')||root.querySelector('h1')).fontSize)||0}},id);assert.equal(m.active,true);assert.equal(m.overflow,false,`${id} must not overflow`);if(layout==='mobile'){assert.ok(m.navTop>=0&&m.navBottom<=renderedHeight+1,`${id} mobile navigation must stay inside rendered viewport (${m.navTop}-${m.navBottom} / ${renderedHeight})`);assert.ok(m.navHeight>=44&&m.navHeight<=100,`${id} mobile navigation height must remain usable and compact`)}else{assert.equal(m.navPosition,'sticky',`${id} desktop navigation must remain the V9.46 sidebar`);assert.ok(m.navTop>=70&&m.navTop<=90,`${id} desktop sidebar must start below header: ${m.navTop}`);assert.ok(m.navBottom<=renderedHeight+6,`${id} desktop sidebar must end at viewport bottom: ${m.navBottom}/${renderedHeight}`);assert.ok(m.navHeight>=850&&m.navHeight<=940,`${id} desktop sidebar height must fill workspace: ${m.navHeight}`);assert.ok(m.navWidth>=200&&m.navWidth<=240,`${id} desktop sidebar width must remain compact: ${m.navWidth}`)}await page.screenshot({path:`${OUT}/${prefix}-${id}.png`,fullPage:false})}}
...
try{
 ...
 const mobile=await boot(browser,{width:390,height:844},true);await snapCore(mobile.page,'iphone','mobile');
