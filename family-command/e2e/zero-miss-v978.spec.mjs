...
assert.doesNotMatch(js,/function priorityDigest/);
assert.match(js,/const pendencies=/,'all independent pendencies stay available');
assert.match(await readFile(new URL('../reference-dashboard-v36.js',import.meta.url),'utf8'),/function todayPanel/,'the canonical renderer owns events without a duplicate child scan');
...
assert.match(sw,/family-command-v131-v11-mobile-briefing/);
...
assert.match(index,/20260922-v1100-preview/,'V11 boot must invalidate the previous product shell cache');
