...
try{
  ...
  await page.waitForFunction(()=>document.documentElement.dataset.fcMobileSchoolDay==='v47');
  ...
  console.log('reference-v47',JSON.stringify(r));
  ...
  assert.equal(r.schoolDisplay,'none','V9.47 hides the desktop week matrix on iPhone');
  assert.notEqual(r.mobileDisplay,'none','V9.47 mobile school-day view must be visible');
  ...
  await browser.close();
  console.log('V9.47 approved iPhone reference regression: ok');
