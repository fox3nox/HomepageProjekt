...
async function runViewport(browser,name,width,height){
  ...
  for(const [id,label] of [['tomorrow','Morgen'],['events','Kalender'],['homework','Aufgaben'],['more','Mehr'],['today','Heute']]){
    const t=Date.now();await page.locator(`.fc9-nav button[data-screen="${id}"]`).click();await page.waitForFunction(x=>document.getElementById(x)?.classList.contains('active'),id);timings[id]=Date.now()-t;assert.ok(timings[id]<700,`${label} tab should respond under 700ms, got ${timings[id]}ms`);assert.equal(await page.locator('.fc9-screen.active').count(),1,`${label}: exactly one active screen`);
  ...
  for(let i=0;i<12;i++){const id=i%2?'today':'tomorrow';await page.locator(`.fc9-nav button[data-screen="${id}"]`).click();await page.waitForFunction(x=>document.getElementById(x)?.classList.contains('active'),id)}
