...
(()=>{
  ...
  function renderTomorrow(){
    ...
    root.innerHTML=`<div class="v6-page fc-tomorrow-page"><div class="v6-page-head"><div><p class="fc-tomorrow-kicker">${esc(fmtLong(date))}</p><h1>Morgen</h1><p>${prepCount?`${prepCount} Punkte zum Vorbereiten oder Beachten`:'Für morgen ist nichts Besonderes offen.'}</p></div><button type="button" class="v6-primary-btn" onclick="openScreen('week');setTimeout(()=>{try{v6SelectWeekDay(${q(date)})}catch(e){}},0)">In Woche</button></div><section class="v6-section"><div class="v6-section-head"><h2>Kinder morgen</h2><span>Los · Beginn · Fertig</span></div><div class="v6-kids">${states.map(s=>kidCard(s,date)).join('')}</div></section>${ev.length?`<section class="v6-section"><div class="v6-section-head"><h2>Termine morgen</h2><button type="button" onclick="openScreen('events')">Alle Termine</button></div><div class="v6-card v6-simple-list">${ev.map(eventRow).join('')}</div></section>`:''}${hw.length?`<section class="v6-section"><div class="v6-section-head"><h2>Bis morgen erledigen</h2><button type="button" onclick="v6OpenHomework()">Alle Aufgaben</button></div><div class="v6-card">${hw.map(homeworkRow).join('')}</div></section>`:''}${!ev.length&&!hw.length&&!states.some(s=>s.pack.length||s.pickup)?'<div class="v6-empty fc-tomorrow-empty">Für morgen ist aktuell nichts zusätzlich vorzubereiten.</div>':''}</div>`;
  ...
  ensureStyle();
  window.renderTomorrow=renderTomorrow;
  ...
  if(typeof window.openScreen==='function'&&!window.__fcTomorrowOpenScreenWrapped){
    ...
    window.openScreen=function(id,...args){const out=raw.call(this,id,...args);if(String(id)==='tomorrow'){try{renderTomorrow()}catch(e){console.error('fc_render_tomorrow',e)}}return out};
    try{openScreen=window.openScreen}catch(e){}
  }

  document.addEventListener('click',e=>{const b=e.target?.closest?.('.navbtn[data-screen="tomorrow"]');if(!b)return;setTimeout(renderTomorrow,0)},true);
  requestAnimationFrame(()=>{if(document.getElementById('tomorrow')?.classList.contains('active'))renderTomorrow()});
  window.__fcTomorrowScreen={version:1,render:renderTomorrow};
})();
