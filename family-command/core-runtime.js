...
function homeworkTasks(){return(data.homework||[]).filter(h=>!h.archived).map(h=>({id:`hw-${h.id}`,personId:h.personId,title:`HAUSAUFGABE: ${[h.subject,h.title].filter(Boolean).join(' · ')}`,date:h.dueDate,note:h.note||'',done:!!h.done}))}
function todoTasks(){return(data.todos||[]).filter(t=>!t.archived&&!window.__fcPersonIdentity?.isMoneyNote?.(t)).map(t=>({id:`todo-${t.id||t.clientRef||t.sourceCommandId}`,personId:t.personId||'',title:t.title||'To-do',date:t.date||'',note:t.note||'',done:!!t.done}))}
function pushSnapshot(){const rules=[];for(const[pid,days]of Object.entries(data.schedules||{}))for(const[day,slots]of Object.entries(days||{}))for(const s of(Array.isArray(slots)?slots:[]))rules.push({id:`${pid}-${day}-${s.start||''}`,personId:pid,title:s.label||'',day:Number(day),time:s.depart||s.start||'',depart:s.depart||'',start:s.start||'',end:s.end||'',note:s.note||''});return{people:(data.people||[]).map(p=>({id:p.id,name:p.name,color:p.color,role:p.role||''})),settings:{travel:0,prep:0,pickup:0,evening:'19:00'},events:(data.events||[]).map(e=>({id:e.id,personId:(e.personIds||[]).join(','),title:e.title,date:e.date,endDate:e.endDate||'',time:e.time||'',end:e.end||'',note:e.note||'',reminderLead:Number.isFinite(Number(e.reminderLead))?Number(e.reminderLead):0})),tasks:[...reminderTasks(),...homeworkTasks(),...todoTasks()],rules}}
