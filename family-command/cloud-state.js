...
let revision=0,busy=false,pending=false,timer=0,status='idle',lastError='',lastSync=0,baseState=null,lastConflictCount=0,uiRetry=0;
...
function mergeStates(remote,local,base=baseState){lastConflictCount=0;base=plain(base)?base:{};const out=mergeObject(base,remote||{},local||{},'state');for(const k of ['people','events','todos','homework','reminders','pendencies'])out[k]=mergeArray(base?.[k],remote?.[k],local?.[k],k);out.schedules=mergeSchedules(base?.schedules,remote?.schedules,local?.schedules);out.common=mergeObject(base?.common||{},remote?.common||{},local?.common||{},'common');return out}
...
async function write(snapshot,base,attempt=0){const {r,j}=await request('POST',{state:snapshot,baseRevision:base,deviceId:deviceId()});if(r.ok&&j.ok){revision=Number(j.revision||base+1);baseState=clone(snapshot);lastSync=Date.now();setStatus('synced','');return true}if(r.status===409&&j.conflict&&valid(j.state)&&attempt<1){revision=Number(j.revision||0);const merged=mergeStates(j.state,snapshot,baseState);replaceState(merged);notifyRender();return write(merged,revision,attempt+1)}throw new Error(j.error||('HTTP '+r.status))}
...
async function bootstrap(){
  ...
  if(LOCAL_TEST&&accessKey()==='test'){revision=1;baseState=clone(current()||{});lastSync=Date.now();setStatus('test','');return{ok:true,test:true,revision}}
  if(!accessKey()||!valid(current())){setStatus('local-only');return{ok:false,reason:'state-or-access-missing'}}
