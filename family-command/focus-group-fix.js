/* Family Command · group simultaneous next actions · 2026-08-25 */
(()=>{
  if(window.__fcFocusGroupFixInstalled)return;window.__fcFocusGroupFixInstalled=true;
  ...
  const homeDeparture=(start,explicit)=>explicit||(start==='07:30'?'07:00':start==='08:20'?'07:55':'');
  ...
  function childActions(date,cur){
    ...
    for(const p of people()){
      ...
      for(const slot of slots){
        const depart=homeDeparture(slot.start,slot.depart),dm=mins(depart),sm=mins(slot.start),em=mins(slot.end);
  ...
  function nextGrouped(){
    const date=today(),cur=nowMins(),all=[...childActions(date,cur),...eventActions(date,cur)].sort((a,b)=>a.m-b.m);
    if(!all.length)return{time:'',title:'Keine zeitkritischen Punkte mehr',sub:'Vergangenes ist ausgeblendet. Offene Aufgaben bleiben weiter unten sichtbar.',left:''};
    const first=all[0],same=all.filter(x=>x.m===first.m);
    if(same.length===1)return{...first,left:leftText(first.m,cur)};

    const allChildren=same.every(x=>x.source==='child');
    const sameKind=allChildren&&same.every(x=>x.kind===same[0].kind);
    if(sameKind){
      const names=[...new Set(same.map(x=>x.person?.name).filter(Boolean))];
      const joined=names.length===2?`${names[0]} & ${names[1]}`:names.length>2?`${names.slice(0,-1).join(', ')} & ${names.at(-1)}`:names[0]||'';
      ...
      return{m:first.m,time:first.time,title:`${joined} ${verb}`,sub,left:leftText(first.m,cur),grouped:true,count:same.length};
    }

    return{m:first.m,time:first.time,title:same.map(x=>x.title).join(' · '),sub:'Mehrere Punkte gleichzeitig',left:leftText(first.m,cur),grouped:true,count:same.length};
  }
  function apply(){
    const focus=document.querySelector('#today .v6-focus');if(!focus)return;
    const x=nextGrouped(),h=focus.querySelector('h2'),p=focus.querySelector('p'),time=focus.querySelector('.v6-focus-time');
    if(h)h.textContent=x.title;if(p)p.textContent=x.sub||'';
    if(x.time){
      let box=time;
      if(!box){box=document.createElement('div');box.className='v6-focus-time';focus.appendChild(box)}
      box.innerHTML=`<b>${x.time}</b>${x.left?`<span>${x.left}</span>`:''}`;
    }else if(time)time.remove();
    focus.dataset.fcGrouped=String(x.grouped?1:0);focus.dataset.fcGroupCount=String(x.count||1);
  }
  function wrap(){
    const fn=window.renderToday;if(typeof fn!=='function'||fn.__fcFocusGrouped)return;
    const wrapped=function(...a){const r=fn.apply(this,a);if(r&&typeof r.then==='function')return r.then(v=>{apply();return v});apply();return r};
    wrapped.__fcFocusGrouped=true;window.renderToday=wrapped;try{renderToday=wrapped}catch(e){}
  }

  wrap();requestAnimationFrame(apply);setTimeout(()=>{wrap();apply()},250);setTimeout(apply,1000);
  const timer=setInterval(()=>{try{apply()}catch(e){}},30000);window.__fcFocusGroupFix={version:1,apply,nextGrouped,timer};
})();
