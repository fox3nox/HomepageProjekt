...
(()=>{
  ...
  function installStyle(){
    ...
    s.textContent=`
      ...
      .fc-doc-proposal .fc-doc-warning{color:#8a4b00}.fc-doc-evidence{display:block;margin-top:5px;padding:7px 9px;border-left:3px solid #9dbce8;border-radius:7px;background:#f5f9ff;color:#3b5576;font-size:11px;line-height:1.45}.fc-doc-evidence b{color:#244a78}.fc-doc-review button{min-height:44px;font:inherit;border-radius:12px;border:1px solid var(--doc-line);padding:10px 12px;cursor:pointer}
  ...
  function showReview(m,review,focusReview=true){
    ...
    const manual=selectedIds(m),rows=review.items.map((x,i)=>{
      ...
      return `<label class="fc-doc-proposal"><input type="checkbox" data-doc-proposal="${i}" ${errors.length?'disabled':''}><span><b>${esc(x.title||'Ohne Titel')}</b><small>${esc(detail)}</small>${x.note?`<small>${esc(x.note)}</small>`:''}${(x.source_quote||x.sourceQuote)?`<small class="fc-doc-evidence"><b>Beleg aus dem Original:</b> „${esc(x.source_quote||x.sourceQuote)}“</small>`:'<small class="fc-doc-warning">Kein Textbeleg geliefert – im Original besonders sorgfältig prüfen.</small>'}${Number(x.confidence||0)<.85?'<small class="fc-doc-warning">Unsichere Erkennung – nur auswählen, wenn du sie im Original geprüft hast.</small>':''}${existing?`<small>Vorhandenen Termin verknüpfen: ${esc(existing.title)} · ${esc([existing.time,existing.end].filter(Boolean).join('–'))}. Seine Angaben bleiben unverändert.</small>`:''}${errors.length?`<small class="fc-doc-warning">${esc(errors.join(' · '))}</small>`:''}</span></label>`;
    }).join('');
  ...
  window.__fcSmartDocumentsHealth={version:'1.4.0',reviewRequired:true,automaticImport:false,filters:true,filterCounts:true,search:true,newestFirst:true,directUpload:true,multiPerson:true,selectAll:true,aiAutoAssign:true,manualOverride:true,autoLink:true,dedupe:true,confidenceThreshold:.85,preservesOriginal:true,semanticNaming:true,sourceEvidence:true};
})();
