/* Family Command · read-only runtime self-test v6.6 · 2026-08-24 */
(()=>{
  ...
  function run(){
    ...
    const todayRelevance=!!window.__fcTodayRelevanceInstalled,tomorrowPreview=!!window.__fcTomorrowPreviewInstalled;
    ...
    const report={version:'6.6',readOnly:true,ok:critical.length===0,screen,activeScreens:active,activeNav,missingFunctions:missing,duplicateIds:dup,brokenHandlers:handlers,weekButtons,eventFilters,expectedFilters,horizontalOverflow:overflow,runtimeErrorCount:state.recentRuntimeErrors.length,smokeTest:window.__fcSmokeTest?{ok:window.__fcSmokeTest.ok,tests:window.__fcSmokeTest.tests}:null,todayRelevance,tomorrowPreview,critical,checkedAt:new Date().toISOString()};
  ...
  window.fcRunSelfTest=run;updateConnection();requestAnimationFrame(()=>{updateConnection();run()});
})();
