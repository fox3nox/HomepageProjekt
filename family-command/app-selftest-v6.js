/* Family Command · read-only runtime self-test v6.17 · 2026-08-25 */
(()=>{
  ...
  function weekControlCount(){const modern=document.querySelectorAll('#week .fc-w-dayhead').length;return modern||document.querySelectorAll('#week .v6-day').length}
  function loadSixNav(){
    if(window.__fcSixNavInstalled||document.querySelector('script[data-fc-six-nav]')){try{window.__fcSixNav?.ensure?.()}catch(e){}return}
    const s=document.createElement('script');s.dataset.fcSixNav='1';s.src='./nav-six-fix.js?v=20260825-pro617';s.onload=()=>{try{window.__fcSixNav?.ensure?.()}catch(e){}scheduleTest()};s.onerror=()=>console.error('fc_six_nav_load');document.body.appendChild(s);
  }
  function loadTomorrowScreen(){
    ...
    const s=document.createElement('script');s.dataset.fcTomorrowScreen='1';s.src='./tomorrow-screen.js?v=20260825-pro617';s.onload=()=>{loadSixNav();scheduleTest()};s.onerror=()=>console.error('fc_tomorrow_screen_load');document.body.appendChild(s);
  }
  function loadPinboard(){
    if(window.__fcPinboardInstalled||document.querySelector('script[data-fc-pinboard]')){try{window.__fcPinboard?.schedule?.()}catch(e){}return}
    const s=document.createElement('script');s.dataset.fcPinboard='1';s.src='./pinboard-dashboard.js?v=20260825-pro617';s.onload=()=>{try{window.__fcPinboard?.schedule?.()}catch(e){}scheduleTest()};s.onerror=()=>console.error('fc_pinboard_load');document.body.appendChild(s);
  }
  ...
  function run(){
    ...
    const weekButtons=screen==='week'?weekControlCount():null,pc=screen==='events'?peopleCount():null,expectedFilters=pc===null?null:pc+1,eventFilters=screen==='events'?document.querySelectorAll('#events .v6-filter').length:null,overflow=activeOverflow(),navOk=screen==='people'?activeNav<=1:activeNav===1,critical=[];
    const todayRelevance=!!window.__fcTodayRelevanceInstalled,tomorrowPreview=!!window.__fcTomorrowPreviewInstalled,tomorrowScreen=!!window.__fcTomorrowScreenInstalled,sixNav=!!window.__fcSixNavInstalled&&!!document.querySelector('.navbtn[data-screen="tomorrow"]'),dailyChecklist=!!window.__fcDailyChecklistInstalled&&!!window.__fcDailyChecklist,todoList=!!window.__fcTodoListInstalled&&!!window.__fcTodo,todoPrint=!!window.__fcTodoPrintInstalled&&!!window.__fcTodoPrint,printPlanner=!!window.__fcPrintPlannerV2Installed&&typeof window.fcPrintDay==='function'&&typeof window.fcPrintWeek==='function',weekOverview=!!window.__fcWeekOverviewV2Installed,pinboard=!!window.__fcPinboardInstalled&&!!window.__fcPinboard;
    if(active!==1)critical.push('active-screen');if(!navOk)critical.push('active-nav');if(missing.length)critical.push('missing-functions');if(handlers.length)critical.push('broken-handlers');if(screen==='week'&&weekButtons!==5)critical.push('week-controls');if(screen==='events'&&expectedFilters!==null&&eventFilters!==expectedFilters)critical.push('event-filters');if(overflow)critical.push('horizontal-overflow');if(!todayRelevance)critical.push('today-relevance');if(!tomorrowPreview)critical.push('tomorrow-preview');if(!sixNav)critical.push('six-nav');if(!dailyChecklist)critical.push('daily-checklist');if(!todoList)critical.push('todo-list');if(!todoPrint)critical.push('todo-print');if(!printPlanner)critical.push('print-planner');if(!weekOverview)critical.push('week-overview');if(!pinboard)critical.push('pinboard');
    if(window.__fcSmokeTest&&window.__fcSmokeTest.ok===false)critical.push('smoke-tests');
    const report={version:'6.17',readOnly:true,ok:critical.length===0,screen,activeScreens:active,activeNav,missingFunctions:missing,duplicateIds:dup,brokenHandlers:handlers,weekButtons,eventFilters,expectedFilters,horizontalOverflow:overflow,runtimeErrorCount:state.recentRuntimeErrors.length,smokeTest:window.__fcSmokeTest?{ok:window.__fcSmokeTest.ok,tests:window.__fcSmokeTest.tests}:null,todayRelevance,tomorrowPreview,tomorrowScreen,sixNav,dailyChecklist,todoList,todoPrint,printPlanner,weekOverview,pinboard,critical,checkedAt:new Date().toISOString()};
  ...
  window.fcRunSelfTest=run;updateConnection();loadTomorrowScreen();loadPinboard();requestAnimationFrame(()=>{updateConnection();run()});
})();