...
function packItems(p,slots,date){
  if(typeof schoolDayFor!=='function')return[];
  return schoolDayFor(p.id,date).items.map(text=>['',text]);
}
...
function label(slots,p,date){if(!slots.length)return{kind:'FREI',icon:'—',time:'Kein Unterricht',note:'',reminders:[],pack:[],dayNotes:[],free:true};const first=slots[0],last=slots[slots.length-1],start=first.start||'',notes=[...new Set(slots.map(x=>String(x.note||'').trim()).filter(Boolean))];const kg=/elia|kindergarten/i.test(`${p?.id||''} ${p?.name||''} ${first.label||''}`);const early=start&&start<'08:10';return{kind:kg?'KG':early?'FRÜH':'SPÄT',icon:kg?'⌂':early?'☀':'◐',time:`${start||'—'}${start?'–':''}${last.end||'—'}`,note:notes.find(n=>!importantNotes(slots).includes(n))||'',reminders:specialReminders(slots),pack:packItems(p,slots,date),dayNotes:importantNotes(slots),free:false}}
function render(){
